#!/usr/bin/env python3
"""Assemble Gen5 schema-v4 supply-chain evidence from offline authorities."""
...
GEN5_ACCESS_BUILD_INPUT_SHA256 = (
    "2283ef4ed48e957c1cee021d7adc0298e82c182a5b6b2702cfc4299578471df3"
)
...
UNSIGNED_SIGNATURE_PLACEHOLDER = hashlib.sha256(
    b"holdfast-schema-v4-unsigned-signature-placeholder"
).hexdigest()
...
def validate_policy_v4_snapshot(raw: bytes, snapshot_path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or policy.get("ceremony") != "holdfast-rikune-successor-v4"
    ):
        fail("Gen5 supply-chain assembly requires successor policy v4")
    predecessor = policy.get("predecessor")
    if (
        not isinstance(predecessor, dict)
        or "completion" in predecessor
        or not HEX64.fullmatch(str(predecessor.get("apply_receipt_sha256", "")))
    ):
        fail("successor policy v4 predecessor completion binding is not exact")
    successor = policy.get("successor")
    if not isinstance(successor, dict) or any(
        successor.get(field) != GEN5_ACCESS_BUILD_INPUT_SHA256
        for field in (
            "source_access_build_input_sha256",
            "access_build_input_sha256",
        )
    ):
        fail("successor policy does not freeze the Gen5 Access build input")
    return policy


def validate_policy_v4_bytes(
    raw: bytes, *, snapshot_parent: Path = Path("/root")
) -> dict[str, Any]:
    with private_snapshot(
        {"successor-policy.json": raw}, parent=snapshot_parent
    ) as snapshot:
        return validate_policy_v4_snapshot(raw, snapshot / "successor-policy.json")


def validate_policy_v4(path: Path) -> dict[str, Any]:
    raw = read_safe_bytes(path, "successor policy")
    return validate_policy_v4_bytes(raw)
...
def validate_access_candidate(
    ...
    if (
        ...
        or receipt["build_input_sha256"] != GEN5_ACCESS_BUILD_INPUT_SHA256
        ...
        fail("Access build receipt schema or Gen5 authority differs")
    ...
    if (
        ...
        fail("candidate Access tree differs from the frozen Gen5 build input")
    ...
    if render_inputs.get("successor-policy.json") != policy_sha256:
        fail("candidate render inputs differ from the exact policy-v4 bytes")
...
def private_snapshot(
    ...
    with tempfile.TemporaryDirectory(
        prefix="holdfast-supply-v4-snapshot-", dir=snapshot_parent
    ) as name:
        root = Path(name)
        root.chmod(0o700)
        if not files:
...
def run_cosign(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="holdfast-cosign-v4-", dir="/root") as name:
        root = Path(name)
        for filename, raw in files.items():
            write_private_file(root, filename, raw)
        command = [
...
def verify_release_manifest(
    ...
    *,
    ...
    if (
        ...
        fail("Strad release manifest differs from the Gen5 release pins")
    run_cosign(
        [
            "verify-blob",
            "--bundle",
...
def validate_previous_release(
    ...
    current_raw = read_safe_bytes(
        current_state_path,
        "Gen4 CURRENT authority",
        allowed_modes={0o600},
        maximum_size=MAX_JSON_BYTES,
    )
    if sha256_bytes(current_raw) != predecessor["current_state_sha256"]:
        fail("Gen4 CURRENT authority differs from policy v4")
    current = load_json_bytes(current_raw, "Gen4 CURRENT authority")
    current = successor_binding.validate_gen4_current(current, estate=estate)
    backup = require_private_directory(
        Path(str(current["backup_dir"])), "Gen4 predecessor backup"
    )
    if backup.parent != Path("/secure/backups") or not backup.name.startswith(
        "holdfast-rikune-"
    ):
        fail("Gen4 predecessor backup is outside the canonical authority")
    successor_binding.validate_gen4_current(current, estate=estate, backup=backup)
    ...
    authority_paths = {
        "apply_receipt": (backup / "APPLY.receipt", MAX_RECEIPT_BYTES),
    ...
    policy_hashes = {
        "apply_receipt": "apply_receipt_sha256",
    ...
    current_hash_fields = {
        ...
        "control": "control_sha256",
        "release_evidence": "release_evidence_sha256",
        "runtime_manifest": "runtime_backup_manifest_sha256",
    }
    for name, policy_field in policy_hashes.items():
        observed = sha256_bytes(authority_raw[name])
        if observed != predecessor[policy_field]:
            fail(f"Gen4 predecessor authority differs: {policy_field}")
        if observed != current[current_hash_fields[name]]:
            fail(f"Gen4 CURRENT authority differs: {current_hash_fields[name]}")

    apply_receipt = successor_binding.validate_gen4_apply_completion(
        authority_raw["apply_receipt"],
        predecessor,
        estate,
        backup,
        current,
    ...
    for relative, name in control_bindings.items():
        if control.get(relative) != sha256_bytes(authority_raw[name]):
            fail(f"Gen4 CONTROL does not bind the previous release: {relative}")
    for receipt_field, name in (
        ...
        if apply_receipt[receipt_field] != sha256_bytes(authority_raw[name]):
            fail(f"Gen4 APPLY completion differs from frozen {receipt_field}")
    ...
    snapshot_files = {
        ...
        "APPLY.receipt": authority_raw["apply_receipt"],
    ...
    with private_snapshot(snapshot_files, parent=snapshot_parent) as snapshot:
        if validate_policy_v4_snapshot(
            current_policy_raw, snapshot / "current-policy.json"
        ) != current_policy:
            fail("snapshot policy v4 differs from predecessor authority")
...
def assemble_release(
    ...
    release.update(
        {
            ...
            "ACCESS_GOVERNANCE_BUILD_INPUT_SHA256": GEN5_ACCESS_BUILD_INPUT_SHA256,
    ...
    if (
        ...
        or access_receipt["build_input_sha256"] != GEN5_ACCESS_BUILD_INPUT_SHA256
    ...
    if len(validator.validate_waivers(waivers, release, issued_at, 4)) != len(waivers):
    ...
    access_candidate = {
        ...
        "build_input_sha256": GEN5_ACCESS_BUILD_INPUT_SHA256,
...
def build_evidence(args: argparse.Namespace) -> int:
    ...
    if source_revision != release_tool_revision:
        fail("Strad and release-tool revisions must be the same Gen5 commit")
    ...
    with private_snapshot(
        ...
        policy = validate_policy_v4_snapshot(
    ...
    if sha256_bytes(trusted_root) != SIGSTORE_TRUSTED_ROOT_SHA256:
        fail("Sigstore trusted root differs from the frozen Gen5 authority")
    ...
    print("schema-v4 evidence and unsigned release env assembled offline")
    return 0
...
def finalize_env(args: argparse.Namespace) -> int:
    ...
    if evidence.get("schema_version") != 4:
        fail("finalize-env only accepts schema-v4 evidence")
    ...
    with private_snapshot(snapshot_files, parent=release_root) as snapshot:
        ...
        if completed.returncode != 0:
            fail("final env failed detached signature or production schema-v4 validation")
    final_inputs = (
        (
            args.unsigned_release_env,
            unsigned_raw,
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="Offline fail-closed Holdfast schema-v4 supply-chain assembler"
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"schema-v4 supply-chain assembler: {error}", file=sys.stderr)
        return 1
